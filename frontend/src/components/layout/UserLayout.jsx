...
import { Home, Wallet, Wifi, Phone, LogOut, Menu, X, User } from 'lucide-react';
...
const UserLayout = () => {
  ...
  const menuItems = [
    ...
    { icon: <Wallet className="w-5 h-5" />, label: 'Fund Wallet', path: '/dashboard/fund-wallet' },
    { icon: <Wifi className="w-5 h-5" />, label: 'Buy Data', path: '/dashboard/buy-data' },
    { icon: <Phone className="w-5 h-5" />, label: 'Buy Airtime', path: '/dashboard/buy-airtime' },
    { icon: <Wallet className="w-5 h-5" />, label: 'Transactions', path: '/dashboard/transactions' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed lg:static inset-y-0 left-0 z-50 w-64 bg-secondary text-white transition-transform duration-300 ease-in-out
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="h-20 flex items-center px-8 border-b border-white/10">
            <Link to="/" className="text-2xl font-bold text-primary">
              VTU<span className="text-white">Hub</span>
            </Link>
          </div>

          {/* User Info (Mobile mostly, but good for sidebar) */}
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-30">
          ...
          <div className="relative">
             <button 
               ...
               className="w-10 h-10 rounded-full bg-orange-50 text-primary border border-orange-100 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 transition-colors"
             >
